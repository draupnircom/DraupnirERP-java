...
/**
 * 这个类是方便控制台输出object，主要应用java反射机制。 因为考虑到使用性和美观性，没有使用无限递归。 而是在toStr方法中加入一个boolean
 * recursion ，是否递归。 当然我们也可以将boolean recursion换成int recursion，控制递归次数。
 * 其实就我使用经验来看，复杂数据toString，用json工具转化成json输出是一个不错的方式。 //这是我用的方式，boolean
 * recursion是否递归 public static int add(int i,boolean recursion){ sum+=i;
 * if(recursion) add(i, false); return sum; } //也可以这样，int recursion表示递归次数 public
 * static int add(int i,int recursion){ sum+=i; if(recursion>0){ recursion--;
 * add(i, recursion); } return sum; }
 * 
 * 
 * @author klguang
 * 
...
public class Printer {
	...
	private static void print(Object obj, String sign, String content) {
		String begin = indent(15, SPLIT_LINE) + "  " + obj.getClass().getSimpleName() + "  >> " + sign + "  "
				+ indent(10, SPLIT_LINE);
